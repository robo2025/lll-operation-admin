    ...
    return [
      {
        ...
        uid: pic[0].id,
        name: pic[0].img_type,
        url: /\/\//.test(pic[0].img_url)
          ? pic[0].img_url
          : FILE_CDN + pic[0].img_url
      }
    ];
  } else {
    return [];
  ...
  handleUploaderChange(key, fileList) {
    ...
    if (key === "cadUrl") {
      ...
      fileList.slice(-1).forEach(file => {
        ...
        if (file.status === "done") {
          ...
          const completeCADS = fileList.filter(
            val => val.status === "complete"
          );
          this.setState({
            cadUrl: completeCADS
          });
    ...
    if (key !== "cadUrl") {
      ...
      if (fileList.length === 0) {
        this.setState({
          pics: removeObjFromArr(
            { img_type: mapImageType[key] },
            pics,
            "img_type"
          )
        });
      ...
      fileList.map(file => {
        if (file.status === "done") {
          ...
          if (key === "a") {
            this.setState({
              pics: replaceObjFromArr(
                {
                  id: pics.length - 100,
                  img_type: "1",
                  img_url: file.response.key
                },
                pics,
                "img_type"
              )
      ...
      <div className={styles["product-info-wrap"]}>
        ...
        <div style={{ float: "left", width: "50%" }}>
          <Form layout="horizontal">
            <FormItem label="所属类目" {...formItemLayout}>
              ...
              })(<Cascader options={catalog} placeholder="请您选择类目" />)}
            </FormItem>
            <FormItem label="品牌" {...formItemLayout}>
              {getFieldDecorator("bno", {
                ...
                >
                  {brands.map(val => (
                    <Option value={val.bno} key={val.bno}>
                      {val.brand_name}
                    </Option>
                  ))}
                </Select>
                ...
                >
                  <Upload
                    ...
                    beforeUpload={currFile =>
                      this.beforeUpload("cadUrl", currFile)
                    }
                    onChange={({ fileList }) => {
                      this.handleUploaderChange("cadUrl", fileList);
                    }}
                    onRemove={currFile => {
                      this.removeCAD(currFile);
                    }}
                    data={{
                      token: uploadToken,
                      key: `product/attachment/cad/${file.uid}.${getFileSuffix(
                        file.name
                      )}`
                    }}
                  >
        ...
        >
          <div style={{ marginBottom: 20 }}>
            <h3>产品图片</h3>
            <small>
              暂时支持格式：JPG/PNG/GIF/BMG/JPGE,文件大小请保持在100KB以内；
            </small>
          </div>
          <Row gutter={24}>
            <Col span={8}>
              <Upload
                ...
                onChange={({ fileList }) => {
                  this.handleUploaderChange("a", fileList);
                }}
                data={{
                  token: uploadToken,
                  key: `product/images/show/${file.uid}.${getFileSuffix(
                    file.name
                  )}`
                }}
              >
            ...
            <Col span={8}>
              <Upload
                ...
                onChange={({ fileList }) => {
                  this.handleUploaderChange("b", fileList);
                }}
                data={{
                  token: uploadToken,
                  key: `product/images/show/${file.uid}.${getFileSuffix(
            ...
            <Col span={8}>
              <Upload
                ...
                onChange={({ fileList }) => {
                  this.handleUploaderChange("c", fileList);
                }}
                data={{
                  token: uploadToken,
                  key: `product/images/show/${file.uid}.${getFileSuffix(
            ...
            <Col span={8}>
              <Upload
                ...
                onChange={({ fileList }) => {
                  this.handleUploaderChange("d4", fileList);
                }}
                data={{
                  token: uploadToken,
                  key: `product/images/show/${file.uid}.${getFileSuffix(
            ...
            <Col span={8}>
              <Upload
                ...
                onChange={({ fileList }) => {
                  this.handleUploaderChange("d5", fileList);
                }}
                data={{
                  token: uploadToken,
                  key: `product/images/show/${file.uid}.${getFileSuffix(
            ...
            <Col span={8}>
              <Upload
                ...
                onChange={({ fileList }) => {
                  this.handleUploaderChange("d6", fileList);
                }}
                data={{
                  token: uploadToken,
                  key: `product/images/show/${file.uid}.${getFileSuffix(
          ...
          <Tabs defaultActiveKey="1">
            <TabPane tab="*产品概述" key="1">
              ...
              <TextArea
                ...
                defaultValue=""
                onChange={e => {
                  this.handleChange("summary1", e.target.value);
                }}
              />
            </TabPane>
            <TabPane tab="*产品详情" key="2">
              <RichEditor
                onChange={html => {
                  this.handleChange("description", html);
                }}
                token={uploadToken}
              />
            </TabPane>
